...
export const useWindowState = ({
  ...
  const [registerResizeCallback, unregisterResizeCallback, resizeCallbacks] =
    useRegisteredCallbacks<ResizeCallback>([]);

  const throttledSetWindowDimensions = throttle(
    stateInterval,
    (newWindowDimensions) => (windowDimensions.current = newWindowDimensions)
  );
...
export const WindowContextProvider: React.FC<IWindowContextProvider> = (
  props
) => {
  const windowState = useWindowState({
    stateInterval: LISTENER_INTERVAL,
    listenerInterval: LISTENER_INTERVAL,
    ...props,
  });
