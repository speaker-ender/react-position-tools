...
export const colorFactory = () => {
  return (Object.keys(COLORS) as (keyof ColorsType)[]).reduce(
    (colorObj, colorName) => {
      colorObj[colorName] = (
        Object.keys(OPACITY) as (keyof typeof OPACITY)[]
      ).reduce((opacityObj, opacityName) => {
        const newOpacityName = `${opacityName}Opacity` as keyof ColorPart<
          typeof OPACITY
        >;
        opacityObj[newOpacityName] = COLORS[colorName].replace(
          "1)",
          `${OPACITY[opacityName]})`
        );
