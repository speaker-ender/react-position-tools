...
export const useSiteState = create<ISiteState>(persist((set, get) => ({
    ...
    setNotification: (notification: Omit<INotification, 'time'>) => {
        set({ notifications: [...get().notifications, { ...notification, time: Date.now() }] });
    },
    dismissNotification: (notification: INotification) => {
        set({ notifications: get().notifications.filter((note) => note !== notification) });
    }
}),
    {
        name: "siteState",
        ...
        partialize: (state) =>
            Object.fromEntries(
                Object.entries(state).filter(([key]) => !["navOpen", "overlayActive", "alert", "lockedScroll"].includes(key))
            ),
