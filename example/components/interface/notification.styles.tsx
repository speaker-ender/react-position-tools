...
export const StyledNotification = styled.div<IStyledNotification>`
  ...
  ${InterfaceBackgroundStyles}
    margin: ${(p) =>
    `calc(${p.theme.spacingProps.defaultSpacing} / 2) ${p.theme.spacingProps.defaultSpacing}`};
  display: grid;
...
export const StyledNotificationWrapper = styled.div<IStyledNotification>`
  ...
  transition: transform ${transition.appearSecondary},
    opacity ${transition.appear}, max-height ${transition.appear},
    min-height ${transition.appear};
  z-index: ${LAYERS.message};
  overflow: hidden;
