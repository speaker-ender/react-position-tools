...
const Alert: React.FC<IAlert> = () => {
    ...
    return (
        <StyledAlert active={!!alert.active}>
            <StyledAlertTitle dangerouslySetInnerHTML={{ __html: alert.title || '' }} />
            <StyledAlertText dangerouslySetInnerHTML={{ __html: alert.text || '' }} />
            <StyledButton dangerouslySetInnerHTML={{ __html: alert.prompt || '' }} onClick={() => setAlertActive(false)}></StyledButton>
        </StyledAlert>
    )
}
